...
[Download PadRemote Host 0.27.0](https://github.com/sakilu/PadRemoteIDE-releases/releases/tag/v0.27.0). No GitHub account is required. This repository contains downloads and release information only; source code is maintained separately.
...
**New in 0.27.0:** A project now has one terminal per AI coding tool instead of one shell you type the tool into. Host starts the tool from the shell's own arguments, detects which tools this computer has — Claude Code, Codex CLI, Grok Build, Antigravity CLI, GitHub Copilot CLI, plus a plain shell — and the management page's new AI Agent section shows each tool's path and version and sets the default tool, extra launch arguments, an executable path or a tool of your own. A tool that is missing says where to install it, and Detect again picks it up without restarting Host. Finish notifications now also cover Gemini CLI and Grok Build. Terminals of tools you are not looking at keep running without sending anything over the connection, and on Mac/Linux with tmux an idle one is closed after 12 hours while its tmux session stays for the next time. The tablet side of this ships in a future App release; updating Host does not change the App.
...
Validation for 0.27.0 includes six platform/CPU builds and archive checks; final Windows EXE/ZIP/Setup Defender scans with security intelligence 1.459.328.0 (real-time protection enabled, no exclusions); Linux amd64 DEB and RPM installation, reinstallation and removal against 0.26.0; ARM64 package structure and macOS PKG structure checks; Go and Flutter regression suites on Windows and in a Linux container, including tmux tests that start a tool and re-attach without starting it twice; a native Android tablet run of the tool switch bar against a disposable Linux Host; anonymous public-download handover from 0.26.0 on Windows and Linux amd64; and the public update channel from 0.12.0 (prompt) and 0.11.1 (automatic). Disposable Windows installer lifecycle QA, macOS/iPad and ARM64 runtime checks were skipped, and a pre-existing scroll-history check in the Android run fails identically on 0.26.0. No Android or iPad store release is included. Cross-compilation and structure checks do not establish native acceptance.
