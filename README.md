...
[Download PadRemote Host 0.24.0](https://github.com/sakilu/PadRemoteIDE-releases/releases/tag/v0.24.0). No GitHub account is required. This repository contains downloads and release information only; source code is maintained separately.
...
**New in 0.24.0:** This Host release preserves the 0.23.0 features and rebuilds all six platform packages after Windows Defender blocked the original 0.23.0 Windows x64 binary as `Trojan:Win32/Bearfoos.B!ml`. The final 0.24.0 Windows x64/ARM64 executables, update ZIPs and Setup EXEs all passed local Defender scans with security intelligence `1.459.324.0`; real-time protection remained enabled and no exclusions were added. Microsoft has not confirmed that the original detection is a false positive or cleared it. Results may differ with later definitions or other environments. The three-minute voice-input limit and Host update API implementation are retained. This is a Host-only release; no new mobile App is published.
...
Validation includes six platform/CPU builds and archive checks; all 24 uploaded asset SHA-256 digests matched local tested files; final Windows Defender scans; Unix installer payload and structure checks with a stubbed macOS shell lifecycle; Linux amd64 DEB/RPM installation, reinstallation and removal against a running 0.22.0 Host; native Windows/Linux amd64 0.22.0 → 0.24.0 update handoffs using anonymous downloads; the stable channel from 0.12.0 (prompt) and 0.11.1 (automatic); and four-language catalog checks. Relevant Host update authorization tests and Go dependency checks passed during the investigation. Unchanged full Go/Flutter suites were not rerun for this version-only rebuild. Disposable Windows installer lifecycle QA, macOS menu bar rendering, and macOS/iPad and ARM64 runtime checks were skipped. Real APNs/FCM delivery remains unverified. No new Android QA build accompanies this release; the Android QA download below remains 0.21.0+31. No Android or iPad store release is included. Cross-compilation and package structure checks do not establish native platform acceptance.
